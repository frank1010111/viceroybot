...
fn markov(_py: Python, m: &PyModule) -> PyResult<()> {
    fn predict(
        ...
        let n_prefix = markov_chain
            .keys()
            .nth(0)
            .expect("prefixes are not formed")
            .len();
    ...
    fn update_chain(
        line: String,
        mut chain: HashMap<Vec<String>, Vec<String>>,
        n_prefix: usize,
        ...
        for i in n_prefix..words.len() {
            ...
            chain
                .entry(prefix)
                .and_modify(|pred| pred.push(prediction.to_string()))
                .or_insert(vec![prediction.to_string()]);
        }
        chain
    }
    ...
    #[pyfn(m)]
    #[pyo3(name = "update_chain")]
    fn update_chain_py<'py>(
        _py: Python<'py>,
        line: &PyString,
        markov_chain: &PyDict,
        n_prefix: usize,
    ) -> PyResult<HashMap<Vec<String>, Vec<String>>> {
        let markov_chain = markov_chain.extract()?;
        let line = line.extract()?;

        let new_chain = update_chain(line, markov_chain, n_prefix);
        Ok(new_chain)
    }
